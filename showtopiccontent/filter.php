...
class filter_showtopiccontent extends moodle_text_filter {
    ...
    function filter($text, array $options = array()) {
        ...
        // if we are not in moodle/course/view, or moodle/mod/lesson/view ignore
        if((strpos($_SERVER['REQUEST_URI'],'moodle/course/view')===false) && 
            (strpos($_SERVER['REQUEST_URI'],'moodle/mod/lesson/view')===false)) {
                return $text;
        }
        ...
        // if we do not have search key, ignore
        if(preg_match($replaceme, $text, $showtopiccontentarray)===false) {
            return $text;
        }
        ...
        $newtext = '';
        // remove the tag from Lesson view in case user places tag in Lesson description
        if(strpos($_SERVER['REQUEST_URI'],'moodle/mod/lesson/view')===false) {
            ...
            foreach($topic_ids as $topicid) {
                ...
                if($cm==null) {
                    ...
                    $lessonclass = $cm->uservisible ? null : array('class' => 'dimmed'); // Hidden modules are dimmed.
                    ...
                    if($lesson->properties()->course != $courseid) {
                        ...
                        while ($pageid != 0) {
                            ...
                            $linkattributes = array('id' => 'lesson-'.$page->id);
                            if($lessonclass!=null) {
                                $linkattributes['class'] = 'dimmed';
                                $linkattributes['style'] = 'pointer-events:none; cursor:default;';
                                $linkattributes['onclick'] = 'return false;';
                            }
                            $ulistarray[] = html_writer::link($url, format_string($page->title, true), $linkattributes);
                        }
                    }
                }
        
                if($lessonclass!=null) {
                    $prehtml = "<div style='".$predivcss."' class='dimmed lesson-list-".$cmid."'>";
                } else {
                    $prehtml = "<div style='".$predivcss."' lesson-list-".$cmid."'>";
                }
                $posthtml = "</br></div>";
